...
/**
 * @brief Configure node as anchor with given options. BLE option canâ€™t be
 ...
 * Encryption canâ€™t be enabled if encryption key is not set.
...
/**
 * @brief Configure node as tag with given options. BLE option canâ€™t be enabled
...
extern void kalman_init(kalman2_state *state, float *init_x, float (*init_p)[2]);
extern float kalman_filter(kalman2_state *state, float z_measure);
